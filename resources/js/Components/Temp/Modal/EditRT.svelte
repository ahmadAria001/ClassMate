<script lang="ts">
    ...
    const { form, isSubmitting, errors } = createForm<z.infer<typeof UpdateRT>>(
        {
            extend: validator<z.infer<typeof UpdateRT>>({ schema: UpdateRT }),
...
<BaseModal bind:showState>
    <form method="post" use:form>
        ...
        <input
            ...
            class="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-amber-600 hover:bg-amber-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-amber-400"
        />
    </form>
</BaseModal>
