<script lang="ts">
    ...
    import { DeleteCiviliant } from "../../../schema";
    ...
    const { form, isSubmitting, errors } = createForm<
        z.infer<typeof DeleteCiviliant>
    >({
        extend: validator<z.infer<typeof DeleteCiviliant>>({
            schema: DeleteCiviliant,
        }),
        onSubmit: async (values) => {
            console.log(values);

            const response = await axios.delete("/api/civiliant", {
                headers: {
                    Authorization: `Bearer ${getCookie("token")}`,
                },
                data: {
